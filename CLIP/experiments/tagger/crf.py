...
def log_sum_exp(inp, m_size):
    ...
    max_score = torch.gather(inp, 1, idx.view(-1, 1, m_size)).view(
        -1, 1, m_size
    )  # batch_size * 1 * hidden_dim
    return max_score.view(-1, m_size) + torch.log(
        torch.sum(torch.exp(inp - max_score.expand_as(inp)), 1)
    ).view(-1, m_size)
...
class CRF(nn.Module):
    ...
    def _forward_alg(self, feats, mask):
        ...
        # s(x, *, y, 1) = s(*, y) + s(x, y)
        partition = (
            scores[0, :, self.START_TAG_IDX, :].clone().view(batch_size, tag_size, 1)
        )
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        for idx, cur_values in enumerate(scores[1:]):
            ...
            cur_bp.masked_fill_(
                mask[idx + 1].view(batch_size, 1).expand(batch_size, tag_size).bool(), 0
            )
            back_points.append(cur_bp)

        partition_history = (
            torch.cat(partition_history)
    ...
    def _score_sentence(self, scores, mask, tags):
        ...
        end_transition = (
            self.transitions[:, self.END_TAG_IDX]
            .contiguous()
            .view(1, tag_size)
