...
import nltk
import os
import data.i2b2.i2b2_tools as i2b2_tools
...
class Document:
    def __init__(self, txt, con=None):
        ...
        if con:
            ...
            self._labels = tok_concepts_to_labels(self._tok_sents, self._tok_concepts)
...
def read_i2b2(txt, con):
    ...
    with open(txt) as f:
        ...
        for sentence in sentences:
            sent = i2b2_tools.clean_text(sentence.rstrip())
            ...
            # normalize tokens
            normed_toks = i2b2_tools.normalize_tokens(toks)
...
def tok_labels_to_concepts(tokenized_sents, tok_labels):
    ...
    for i, labs in enumerate(tok_labels):
        ...
        for start in begins:
            ...
            while (
                (end < N - 1)
                and tok_labels[i][end + 1].startswith("I")
                and tok_labels[i][end + 1][1:] == label
            ):
                end += 1
    ...
    #'''
    for lineno, (test, gold, sent) in enumerate(
        zip(test_tok_labels, tok_labels, tokenized_sents)
    ):
